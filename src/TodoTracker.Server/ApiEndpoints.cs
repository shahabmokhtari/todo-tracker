...
public sealed record ConnectionDto(string BaseUrl, string LaunchUrl, string McpUrl, string Token, object McpConfig);
...
internal static class ApiEndpoints
{
    ...
    internal static ConnectionDto Connection(ApiToken token, TodoTrackerServerOptions options)
    {
        ...
        return new ConnectionDto(baseUrl, $"{baseUrl}/auth?token={Uri.EscapeDataString(token.Value)}", mcpUrl, token.Value, config);
    }
